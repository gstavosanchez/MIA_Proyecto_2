...
CREATE TABLE DetalleUsuario(
    detalle_usuario_ID NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,
    membresia_ID NUMBER NOT NULL,
    usuario_ID NUMBER NOT NULL,
    temporada_ID NUMBER NOT NULL,
    CONSTRAINT detalle_usuario_pk PRIMARY KEY(detalle_usuario_ID),
...
CREATE OR REPLACE PROCEDURE sp_insert_detalle_usuario(
    p_usuario IN VARCHAR,
    p_temporada IN VARCHAR,
    p_membresia IN VARCHAR
)
IS
    v_usuario_ID NUMBER := 0;
    v_temporada_ID NUMBER := 0;
    v_membresia_ID NUMBER := 0;
    v_count NUMBER := 0;
BEGIN
    ...
    WHERE u.USER_NAME = p_usuario
    AND t.NOMBRE =p_temporada
    AND m.NOMBRE = p_membresia;
    IF v_count = 0 THEN
        SELECT USUARIO_ID INTO v_usuario_ID FROM USUARIO WHERE USER_NAME = p_usuario;
        SELECT TEMPORADA_ID INTO v_temporada_ID FROM TEMPORADA WHERE NOMBRE = p_temporada;
        SELECT MEMBRESIA_ID INTO v_membresia_ID FROM MEMBRESIA WHERE NOMBRE = p_membresia;

        IF ((v_usuario_ID > 0 AND v_usuario_ID IS NOT NULL) AND (v_temporada_ID > 0 AND v_temporada_ID IS NOT NULL) 
            AND(v_membresia_ID > 0 AND v_membresia_ID IS NOT NULL)) THEN
            INSERT INTO DetalleUsuario(USUARIO_ID,TEMPORADA_ID,MEMBRESIA_ID)
            VALUES (v_usuario_ID,v_temporada_ID,v_membresia_ID);
        END IF;
    END IF;
END sp_insert_detalle_usuario;
/* ================= ===================== VISTAS ================= ===================== */
CREATE OR REPLACE VIEW view_total_por_temporada AS 
SELECT SUM(m.PRECIO) AS TOTAL,t.Nombre AS TIER
FROM DetalleUsuario du
INNER JOIN USUARIO u ON du.USUARIO_ID = u.USUARIO_ID
INNER JOIN TEMPORADA t ON du.TEMPORADA_ID = t.TEMPORADA_ID
INNER JOIN MEMBRESIA m ON du.MEMBRESIA_ID = m. MEMBRESIA_ID
...
CREATE OR REPLACE VIEW view_no_user_por_membresia_temporada AS 
SELECT COUNT(u.usuario_ID)AS NO_Usuarios,t.NOMBRE AS Season,m.NOMBRE as TIER
FROM DetalleUsuario du
INNER JOIN USUARIO u ON du.USUARIO_ID = u.USUARIO_ID
INNER JOIN TEMPORADA t ON du.TEMPORADA_ID = t.TEMPORADA_ID
INNER JOIN MEMBRESIA m ON du.MEMBRESIA_ID = m.MEMBRESIA_ID
...
SELECT * FROM view_total_por_temporada where TIER = '2020-Q11';
SELECT * FROM view_no_user_por_membresia_temporada WHERE Season = '2020-Q11'
